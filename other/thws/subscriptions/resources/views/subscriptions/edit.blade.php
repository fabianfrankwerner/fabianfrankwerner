...
<div class="mb-3 row">
            <label for="mediatype_id" class="col-md-4 col-form-label text-md-end text-start">Medientyp</label>
            <div class="col-md-6">
                <input type="text" class="form-control @error('mediatype_id') is-invalid @enderror" id="mediatype_id" name="mediatype_id" value="{{ $subscription->mediatype_id }}">
                @if ($errors->has('mediatype_id'))
                <span class="text-danger">{{ $errors->first('mediatype_id') }}</span>
                @endif

				<span>{{ $subscription->mediatype }}<br>
				<select name="mediatype_id">
					@foreach ($mts as $mt)
						<option value="{{$mt->id}}" @selected($mt->id == $subscription->mediatype_id) >{{$mt->name}}</option>
					@endforeach
				</select>
            </div>
        </div>
