...
\begin{enumerate}
  \item \textbf{Treatment Condition (Labeled)}: Participants were shown an advertisement containing a conspicuous text badge in the bottom-right corner reading ``AI-generated'' (German: ``KI-generiert''). This label was designed according to the recommendations by Gamage et al. \cite{gamageLabelingSyntheticContent2025}, who identified ``AI-generated'' as the most effective phrase for user recognition. The label's design followed a standardized format (semi-transparent background, white Roboto font, size 32px) to simulate realistic social media disclosure practices.
...
Participants were asked to rate the extent to which specific adjectives described the advertisement they had just viewed. The items were measured on a 5-point Likert scale ranging from 1 = "Strongly disagree" to 5 = "Strongly agree" (German: Stimme überhaupt nicht zu to Stimme voll und ganz zu).
...
\begin{itemize}
    ...
    \item \textbf{Clarity:} Measured by the items \textit{clear}, \textit{understandable}, and \textit{not confusing}. These items evaluate the comprehensibility of the message itself. Note: The original item "confusing" from Huschens et al. (2023) was reverse-coded in the questionnaire design (presented as "not confusing") to ensure consistent polarity across all scale items.
    \item \textbf{Engagement:} Measured by the items \textit{interesting} and \textit{draws attention}. These items assess the ability of the message to capture the viewer's interest.
\end{itemize}

To minimize response bias, the order of these 11 items was randomized for each participant within the survey.
...
To investigate the potential moderating effect of pre-existing beliefs (Hypotheses H5-H8), the study measured the participants' General Attitude towards Artificial Intelligence.

This variable was assessed prior to the experimental stimulus to avoid priming effects. A single-item global measure was used, asking participants: "How is your general attitude towards Artificial Intelligence?" (German: "Wie ist Ihre generelle Einstellung zu Künstlicher Intelligenz?").

The response was recorded on a 5-point Likert scale:

\begin{itemize}
    \item Very negative \textit{(Sehr negativ)}
    \item Rather negative \textit{(Eher negativ)}
    \item Neutral \textit{(Neutral)}
    \item Rather positive \textit{(Eher positiv)}
    \item Very positive \textit{(Sehr positiv)}
\end{itemize}
